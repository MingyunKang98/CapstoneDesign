...
img = cv2.warpPerspective(src, matrix, (width, height))
#
rows = 1
cols = 2
...
edges = cv2.Canny(gray,150,220,apertureSize = 3)
...
ax3.set_title('Canny Edge')
ax3.axis("off")
ax4 = fig.add_subplot(rows,cols,2)
ax4.imshow(cv2.cvtColor(src, cv2.COLOR_BGR2RGB))
ax4.set_title('Hough Line')
ax4.axis("off")
plt.show()
